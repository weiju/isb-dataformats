...
object MeasurementSpec extends Specification {
  ...
  "Measurement" should {
    doBefore {
      dataMatrix = DataMatrixReader.createFromFile(new File(getClass.getResource("/matrix_output").getFile))
      oligoMap = new OligoMapDatabase(new File(getClass.getResource("/Slide_Templates").getFile)).latestMap
    }
